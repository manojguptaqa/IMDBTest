...
public class Wait {
	...
	public static void untilElementIsVisible(WebDriver driver,WebElement element){
		untilElementIsVisible(driver,element,FileReaderManager.getInstance().getConfigReader().getImplicitlyWait());
		 }
	
	public static void untilElementIsVisible(WebDriver driver, WebElement element,Long timeoutInSeconds){
		 until(driver, (d) ->
		 {
		 Boolean isElementisible = element.isDisplayed();
		 if (!isElementisible) System.out.println("Element still not visible");
		 return isElementisible;
		 }, timeoutInSeconds);
		 }
